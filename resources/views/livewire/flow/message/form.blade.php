<div>
    <x-collapse wire:model="show" separator class="bg-base-200">
        <x-slot:heading wire:click='toggleCollapseForm'>{{$show ? "Fechar" : "Abrir"}} formúlario de criação
        </x-slot:heading>
        <x-slot:content>
            <div class="">
                <div class="mb-3">
                ...
                <x-form class="" wire:submit='handleSubmit'>
                    ...
                    @case('text')
                    <div class="mb-3">
                        <x-textarea max=500 label="Conteudo" wire:model="text"
                            placeholder="Digite o conteudo da mensagem" hint="O texto da mensagem" rows="1" inline />
                    </div>
                    @break

                    @case('image')
                    <x-input label="Texto de apoio da imagem" wire:model="text"
                        hint="Mensagem que será enviada junto à imagem" />
                    <x-file wire:model="image" label="Imagem" hint="Somente imagens" accept="image/*" />
                    @break

                    @case('video')
                    <x-input label="Texto de apoio do vídeo" wire:model="text"
                        hint="Mensagem que será enviada junto ao video" />
